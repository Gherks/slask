...
Scenario: Can create tournament
	...
	Then tournament 0 should be valid with name: "GSL 2019"
	
Scenario: Cannot create tournament with empty name
	When a tournament named "" has been created
	Then tournament 0 should be invalid

Scenario: Cannot create tournament with name already in use no matter letter casing
	Given a tournament named "GSL 2019" has been created
	When a tournament named "Gsl 2019" has been created
	Then tournament 1 should be invalid

Scenario: Can rename tournament
	Given a tournament named "GSL 2019" has been created
	When tournament 0 is renamed to: "Homestory Cup XX"
	Then tournament 0 should be valid with name: "Homestory Cup XX"

Scenario: Cannot rename tournament to empty name
	Given a tournament named "GSL 2019" has been created
	When tournament 0 is renamed to: ""
	Then tournament 0 should be valid with name: "GSL 2019"
...
Scenario: Cannot rename non-existing tournament
	When fetching tournament by tournament id: 00000000-0000-0000-0000-000000000000
	Then fetched tournament 0 should be invalid

Scenario: Can fetch tournament by id
	Given a tournament named "GSL 2019" has been created
	When fetching tournament 0 by tournament id
...
Scenario: Can add user to tournament with user service
	...
	When users "Stålberto" is added to tournament "GSL 2019"
	Then tournament 0 should contain valid betters with names: "Stålberto"
...
Scenario: Can only add user to tournament once
	...
	When users "Stålberto" is added to tournament "GSL 2019"
	Then better count in tournament 0 should be 3

Scenario: Can fetch all betters in tournament by tournament id
	Given a tournament named "GSL 2019" has been created with users "Stålberto, Bönis, Guggelito" added to it
	When fetching betters from tournament 0 by tournament id
	Then tournament 0 should contain valid betters with names: "Stålberto, Bönis, Guggelito"
